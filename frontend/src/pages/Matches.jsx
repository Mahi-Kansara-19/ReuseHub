...
const Matches = () => {
  ...
  const [user, setUser] = useState(() => {
    try {
      ...
      console.error("Failed to parse user storage in Matches:", err);
      return {};
    }
  });
  ...
  const loadMatches = async () => {
    try {
      ...
      console.error("Error loading matches:", error.response?.data || error);
    }
  };
  ...
  const updateMatchStatus = async (id, status) => {
    try {
      ...
      loadMatches(); // refresh from server to ensure referential integrity sync
    } catch (error) {
              ...
              >
                ...
                <div className="mt-auto pt-4 border-t border-slate-100">
                  {match.status === "new" && (
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-2 bg-[#4A7538] hover:bg-[#5b8c45] text-white py-3 rounded-xl font-bold transition shadow-sm cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-2 bg-rose-50 hover:bg-rose-100 text-rose-700 py-3 rounded-xl font-bold transition cursor-pointer border border-rose-100"
                      >
                  ...
                  {match.status === "pending" && (
                    <div className="text-center py-2.5 px-4 rounded-xl border border-amber-200 bg-amber-50 text-amber-700 font-bold text-sm">
